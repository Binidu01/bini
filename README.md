<div align="center">
  ...
  <p>
    ...
    <a href="https://www.npmjs.com/package/bini-ssg"><img src="https://img.shields.io/npm/v/bini-ssg?color=00cfff&labelColor=040a16&label=bini-ssg" alt="bini-ssg" /></a>
...
Bini.js is a React framework for building modern applications that run natively across web, desktop, and mobile — all from a single codebase. It gives you file-based routing, [Hono](https://hono.dev)-powered API routes, per-route metadata, a zero-dependency production server, static site generation, native plugin wiring, one-command deployment, and a polished dev experience all wired together and ready to deploy anywhere.
...
my-app/
├── src/
│   ├── main.tsx              # React entry point + SSG render() export
...
### `bini-ssg` · [npm](https://www.npmjs.com/package/bini-ssg)

A Vite plugin that pre-renders every route — static and dynamic alike — to static HTML as part of `npm run build`, no separate export command or export mode needed. It loads the `render()` function exported from `src/main.tsx` and calls it for each route from `bini-router`'s manifest, producing real server-rendered markup rather than a client-only shell; the client then hydrates it with `hydrateRoot` on load. Dynamic routes that can't be pre-rendered fall back to a shell page automatically. Already wired into every scaffold — nothing to add to `vite.config.ts`.
